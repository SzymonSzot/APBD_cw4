...
public sealed class ZadaniaLinq
{
    ...
    {
        /*// Query syntax
        IEnumerable<string> query = from s in DaneUczelni.Studenci
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        var method = DaneUczelni.Prowadzacy.
            Join(DaneUczelni.Przedmioty,
                ...
                (prow, przedm) => prow)
            .GroupBy(prowadzacy => $"{prowadzacy.Imie}, {prowadzacy.Nazwisko}")
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        var method = DaneUczelni.Zapisy.
            ...
            .Where(r => r.OcenaKoncowa != null)
            .GroupBy(r=>$"{r.Imie} {r.Nazwisko}")
            .Select(g=>$"{g.Key}, {g.Max(z => z.OcenaKoncowa)}");
    ...
    public IEnumerable<string> Wyzwanie01_StudenciZWiecejNizJednymAktywnymPrzedmiotem()
    {
        var method = DaneUczelni.Zapisy.
            ...
            .GroupBy(r=>$"{r.Imie} {r.Nazwisko}")
            .Where(g=>g.Count()>1)
            .Select(g=>$"{g.Key}, {g.Count(z => z.CzyAktywny)}");
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        var method = DaneUczelni.Zapisy.Join(DaneUczelni.Przedmioty,
                z => z.PrzedmiotId,
                p => p.Id,
                (z, p) => new { p.Nazwa, z.OcenaKoncowa, p.DataStartu })
            .Where(r => r.DataStartu.Year.Equals(2026))
            .GroupBy(r => r.Nazwa)
            .SkipWhile(g => g.All(z => z.OcenaKoncowa != null))
            .Select(g => $"{g.Key}");
